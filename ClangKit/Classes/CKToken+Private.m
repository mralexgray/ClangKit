...
- ( id )initWithCXToken: ( CXToken )token translationUnit: ( CKTranslationUnit * )translationUnit
{
    ...
    if( ( self = [ self init ] ) )
    {
        ...
        _sourceLocation =[CKSourceLocation.alloc 
         initWithPointerData1: ( void * )location.ptr_data[ 0 ] pointerData2: ( void * )location.ptr_data[ 1 ] intData: location.int_data ];
        _cursor         =[CKCursor.alloc 
         initWithLocation: _sourceLocation translationUnit: translationUnit ];
    }
